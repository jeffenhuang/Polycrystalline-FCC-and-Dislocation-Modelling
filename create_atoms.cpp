...
void CreateAtoms::add_lattice()
{
  ...
  if (style == REGION)
  {
	  ncell = domain->regions[nregion]->ask_ncell();
  }

  if ( ncell > 1 )
  {
      double latticex[3] = {0.0};
      double latticey[3] = {0.0};
      double latticez[3] = {0.0};
      domain->lattice->getOrientx(latticex);
      domain->lattice->getOrienty(latticey);
      domain->lattice->getOrientz(latticez);

      for (int inx = 0; inx < ncell; inx++ )
      {
	      //int atype = round(rand()%7);
          //for(int idxBasis = 0; idxBasis < nbasis;idxBasis++)
      	  //{
      	  //    basistype[idxBasis]=atype;
          //}
          //Reset lattice orientation
		  double alfa  = RAND*PI;
		  double beta  = RAND*PI;
		  double gamma = RAND*PI;

		  double newx[3] = {0.0};
		  double newy[3] = {0.0};
		  double newz[3] = {0.0};

		  //The following if from wikipedia.org
		  //https://zh.wikipedia.org/wiki/%E6%97%8B%E8%BD%AC%E7%9F%A9%E9%98%B5
		  //newx[0] = cos(alfa)*cos(gamma)-cos(beta)*sin(alfa)*sin(gamma);
		  //newx[1] = -cos(beta)*cos(gamma)*sin(alfa)-cos(alfa)*sin(gamma);
		  //newx[2] = sin(alfa)*sin(beta);
		  //newy[0] = cos(gamma)*sin(alfa)+cos(alfa)*cos(beta)*sin(gamma);
		  //newy[1] = cos(alfa)*cos(beta)*cos(gamma) - sin(alfa)*sin(gamma);
		  //newy[2] = -cos(alfa)*sin(beta);
		  //newz[0] = sin(beta)*sin(gamma);
		  //newz[1] = cos(gamma)*sin(beta);
		  //newz[2] = cos(beta);

		  //The following is calculated by myself
		  newx[0] = cos(gamma)*cos(beta);
		  newx[1] = cos(gamma)*sin(beta)*sin(alfa)-sin(gamma)*cos(alfa);
		  newx[2] = cos(gamma)*sin(beta)*cos(alfa)+sin(gamma)*sin(alfa);
		  newy[0] = sin(gamma)*cos(beta);
		  newy[1] = sin(gamma)*sin(beta)*sin(alfa)+cos(gamma)*cos(alfa);
		  newy[2] = sin(gamma)*sin(beta)*cos(alfa)-cos(gamma)*sin(alfa);
		  newz[0] = -sin(beta);
		  newz[1] = cos(beta)*sin(alfa);
		  newz[2] = cos(beta)*cos(alfa);

          domain->lattice->setOrientx(newx);
          domain->lattice->setOrienty(newy);
          domain->lattice->setOrientz(newz);
		  domain->lattice->setup_transform();
          int i, j, k, m;
          ...
          for (k = klo; k <= khi; k++)
              ...
              if (!domain->regions[nregion]->match(inx, x[0], x[1], x[2])) continue;
      ...
      domain->lattice->setOrientx(latticex);
      domain->lattice->setOrienty(latticey);
      domain->lattice->setOrientz(latticez);
	  domain->lattice->setup_transform();
  }
  else
  {
