...
Region::Region(LAMMPS *lmp, Region *region) : Pointers(lmp)
{
    scaleflag = region->scaleflag;
    xscale = region->xscale;
    yscale = region->yscale;
    zscale = region->zscale;
}
...
int Region::ask_ncell()
{
    return ncell;
}
/* ----------------------------------------------------------------------
inside = 1 if x,y,z is inside or on surface idx
inside = 0 if x,y,z is outside and not on surface idx
------------------------------------------------------------------------- */

int Region::isInside(int idx, double x, double y, double z)
{
    double rx, ry, rz;
    int pid;
    if (inside(x,y,z))
    {
		con_mono->find_voronoi_cell(x, y, z, rx, ry, rz, pid);
		if (pid == idx)
		{
			return 1;
		}
...
int Region::match(int idx, double x, double y, double z)
{
    if (dynamic) inverse_transform(x, y, z);
    return !(isInside(idx, x, y, z) ^ interior);
}
...
void Region::options(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    {
        if (iarg + 4 > narg)
            error->all(FLERR, "legal region command");

        //if (strstr(arg[iarg + 1], "v_") != arg[iarg + 1])
        //    error->all(FLERR, "Illegal region command");
        if (strcmp(arg[iarg+1],"NULL") != 0)
          xcell = force->inumeric(FLERR, arg[iarg + 1]);
		if (strcmp(arg[iarg+2],"NULL") != 0)
		  ycell = force->inumeric(FLERR, arg[iarg + 2]);
		if (strcmp(arg[iarg+3],"NULL") != 0)
		  zcell = force->inumeric(FLERR, arg[iarg + 3]);
