...
Lattice::Lattice(LAMMPS *lmp, int narg, char **arg) : Pointers(lmp)
{
  ...
  while (iarg < narg) {
    if (strcmp(arg[iarg],"origin") == 0) {
      ...
      orient[0] = force->numeric(FLERR,arg[iarg+2]);
      orient[1] = force->numeric(FLERR,arg[iarg+3]);
      orient[2] = force->numeric(FLERR,arg[iarg+4]);
      iarg += 5;
...
void Lattice::getOrientx( double (&xx)[3])
{
	xx[0] = orientx[0];
    xx[1] = orientx[1];
    xx[2] = orientx[2];
}
void Lattice::getOrienty( double (&yy)[3])
{
	yy[0] = orienty[0];
    yy[1] = orienty[1];
    yy[2] = orienty[2];
}
void Lattice::getOrientz( double (&zz)[3])
{
	zz[0] = orientz[0];
    zz[1] = orientz[1];
    zz[2] = orientz[2];
...
void Lattice::setOrientx( double xx[3])
{
	orientx[0] = xx[0];
    orientx[1] = xx[1];
    orientx[2] = xx[2];
...
void Lattice::setOrienty( double yy[3])
{
	orienty[0] = yy[0];
    orienty[1] = yy[1];
    orienty[2] = yy[2];
}
void Lattice::setOrientz( double zz[3])	
{
	orientz[0] = zz[0];
    orientz[1] = zz[1];
    orientz[2] = zz[2];
