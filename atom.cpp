...
Atom::~Atom()
{
  ...
  // delete dislocation
  //memory->destroy(dislocation_p);
  //memory->destroy(dislocationIndex);
  //memory->destroy(dislocationAtom);
  ...
  for (int i = 0; i < ndislocation; i++)
  {
	 //delete dislocations[i];
	  dislocations[i] = NULL;
  }
  //memory->sfree(dislocation_p);
...
/* ----------------------------------------------------------------------
   Dislocation related functions
------------------------------------------------------------------------- */
/************************************************************************/
/* Function Name: add_dislocation                                       */
/* Description:   Add a dislocation into system                         */
/* Input:		  char* id dislocation id	                            */
/* Output:		                                                        */
/* Return:		  none                                                  */
/************************************************************************/
void Atom::add_dislocation(Dislocation* dislocationIn)
{
	if (find_dislocation(dislocationIn->id) >= 0)
		error->all(FLERR, "Reuse of molecule template ID");

	int nprevious = ndislocation;
	ndislocation += 1;

	if (dislocations == NULL)
	{
		dislocations = (Dislocation **)
			memory->smalloc(ndislocation*sizeof(Dislocation *), "atom::dislocations");
	}
	else
	{
		dislocations = (Dislocation **)
			memory->srealloc(dislocations, ndislocation*sizeof(Dislocation *), "atom::dislocations");
	}
	
	dislocations[nprevious] = dislocationIn;
}

/************************************************************************/
/* Function Name: remove_dislocation                                    */
/* Description:   Remove a dislocation from system                      */
/* Input:		  Dislocation* dislocationIn the dislocation 	        */
/* Output:		                                                        */
/* Return:		  none                                                  */
/************************************************************************/
void Atom::remove_dislocation(Dislocation* dislocationIn)
{
	int idx = find_dislocation(dislocationIn->id);
	if (idx >= 0)
	{
		for (int inx = idx; inx < ndislocation-1; inx++)
		{
			dislocations[inx] = dislocations[inx + 1];
		}
		dislocations[ndislocation - 1] = NULL;
		ndislocation -= 1;

		//memory leak if not reallocate for dislocations since one dislocation has been deleted
	}
}

int Atom::find_dislocation(const char * id)
{
	int iDislocation;
	for (iDislocation = 0; iDislocation < ndislocation; iDislocation++)
	if (strcmp(id, dislocations[iDislocation]->id) == 0) return iDislocation;
	return -1;
}
...
//    int peratom_flag = 1;
//    int size_peratom_cols = 0;
//    cutsq = 9;
//
//    int irequest = neighbor->request(this, 0);
...
//    nnearest = NULL;
//    pattern = NULL;
//    int invoked_peratom = update->ntimestep;
//
