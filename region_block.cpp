...
RegBlock::RegBlock(LAMMPS *lmp, int narg, char **arg) : Region(lmp, narg, arg)
{
  ...
  //if ncell greater than zero create voronoi cell and add to domain.
  if ( ncell > 0 )
  {
      delete con_mono;
      con_mono = new voro::container(xlo, xhi, ylo, yhi, zlo, zhi, xcell, ycell, zcell, false, false, false, 8);

	  int inx = 0;
	  double (*points)[3] = (double((*)[3]))malloc( ncell*3*sizeof(double));
	  double xlength = (xhi-xlo)/xcell;
	  double ylength = (yhi-ylo)/ycell;
	  double zlength = (zhi-zlo)/zcell;
	  double randx = xlength/2.5;
	  double randy = ylength/2.5;
	  double randz = zlength/2.5;
      for (int idx = 0; idx < xcell; idx++ )
      {
          for( int idy = 0; idy < ycell; idy++)
          {
              for (int idz = 0; idz < zcell; idz++)
              {
                  points[inx][2] = zlo+zlength/2+zlength*idz+RAND*randz;
                  points[inx][1] = ylo+ylength/2+ylength*idy+RAND*randy;
                  points[inx][0] = xlo+xlength/2+xlength*idx+RAND*randx;

                  con_mono->put(inx++, points[inx][0],points[inx][1],points[inx][2]);
              }
          }
      }
	  free(points);
      con_mono->compute_all_cells();
  }
}
